...
def write_docx():
    ...
    output_path = to_docx(
        st.session_state["template"]["bytes_data"],
        statement,
        skills_str,
        experiences,
    )
...
def export_docx() -> None:
    ...
    if len(st.session_state["experiences"]) == 0:
        ...
        return
    st.markdown(
        "<h2 style='text-align: center;'>Export resume</h2>",
    ...
    if len(st.session_state["project_choices"]) == 0:
        project_choices = st.multiselect(
            "Select relevant projects", options.keys(), default=options
        )
    else:
        project_choices = st.multiselect(
    ...
    if (
        "template" in st.session_state
        and "project_choices" in st.session_state
    ):
        ...
        st.write(f"Current template: __{template_name}__")
